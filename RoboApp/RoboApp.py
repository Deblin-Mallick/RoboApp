...
class SoccerRobotController:
    ...
    def _setup_gui(self):
        ...
        # Place the logging button at the bottom right using .place()
        self.toggle_log_btn = ttk.Button(self.root, text="Disable Logging", command=self._toggle_logging)
        self.toggle_log_btn.place(relx=0.97, rely=0.93, anchor='ne')  # adjust as needed
    ...
    def _network_loop(self):
        ...
        while not self.stop_flag.is_set():
            if not self.connected:
                time.sleep(0.2)  # Reduce CPU usage when disconnected
                continue
            try:
    ...
    def _start_gui_update_loop(self):
        def update_gui():
            for _ in range(20):  # Process max 20 items per cycle
                try:
